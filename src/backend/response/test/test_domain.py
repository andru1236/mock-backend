...
@pytest.fixture(
    params=["base", "with_tracking", "with_trancking_and_routes"], scope="function"
)
...
def test_create_respose_from_dict_default(dict_test_case):
    ...
    if len(result.tracking_assignations):
        assert isinstance(
            result.tracking_assignations[0], dict_test_case.type_class_tracking
        )
        if len(result.tracking_assignations[0].routes):
            assert isinstance(
                result.tracking_assignations[0].routes[0],
                dict_test_case.type_class_tracking_route,
            )
